...
test('ad date conversion', async () => {
  try {
    const ogDate = moment();
    const adDate = ad.other().toADDate(ogDate.format('YYYY-MM-DD HH:mm:ss'));
    const convertedDate = ad.other().fromADDate(adDate);
    expect(ogDate.isSame(moment(convertedDate), 'day')).toBeTruthy();
  } catch (err) {
    expect(err).toBeUndefined();
  }
});
